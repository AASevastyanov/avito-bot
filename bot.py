...
async def start_command(message: Message):
    # 1) Отправляем фотку
    photo_file = FSInputFile("images/start_img.jpg")  # <-- меняешь на свой реальный путь к example.png
    await message.answer_photo(
        photo=photo_file,
        caption="🎄 Приветствую!\n\n" 
                "Вот вам картинка для настроения."
    )
    
    # 2) Отправляем обычное текстовое сообщение с кнопками
